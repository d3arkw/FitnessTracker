...
async def get_workout_history(db: AsyncSession, current_user: User):
    result = await db.execute(
        select(Workout.title, Workout.date, func.count(func.distinct(WorkoutSet.exercise_id)).label("quantity"),
               func.sum(WorkoutSet.reps * WorkoutSet.weight).label("volume")).join(WorkoutSet).where(
            Workout.user_id == current_user.id).group_by(Workout.id, Workout.title, Workout.date).order_by(Workout.date.desc()))
    rows = result.all()
    return [WorkoutsHistory(title=row.title, date=row.date, quantity=row.quantity, volume=row.volume or 0)
            for row in rows]
